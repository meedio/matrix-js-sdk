...
export class EncryptionManager {
  ...
  constructor(userId, deviceId, getMemberships, transport, statistics, onEncryptionKeysChanged, parentLogger) {
    ...
    _defineProperty(this, "sendEncryptionKeysEvent", /*#__PURE__*/function () {
      var _ref = _asyncToGenerator(function* (indexToSend) {
        ...
        if (!myKeys) {
          _this.e2eeLogger.warn("Tried to send encryption keys event but no keys found!", _this.logContext);
          return;
        }

        // NOTE: not sure why there was no length check so adding a log for debugging
        if (!myKeys.length) {
          _this.e2eeLogger.error("Error: no keys found, but we passed through the check!", _this.logContext);
        }
        if (typeof indexToSend !== "number" && _this.latestGeneratedKeyIndex === -1) {
          _this.e2eeLogger.warn("Tried to send encryption keys event but no current key index found!", _this.logContext);
          return;
        }
        var keyIndexToSend = indexToSend !== null && indexToSend !== void 0 ? indexToSend : _this.latestGeneratedKeyIndex;
        _this.e2eeLogger.info("Try sending encryption keys event. keyIndexToSend=".concat(keyIndexToSend, " (method parameter: ").concat(indexToSend, ")"), _this.logContext);
        ...
        try {
          ...
          _this.logger.info("sendEncryptionKeysEvent participantId=".concat(_this.userId, ":").concat(_this.deviceId, " numKeys=").concat(myKeys.length, " currentKeyIndex=").concat(_this.latestGeneratedKeyIndex, " keyIndexToSend=").concat(keyIndexToSend), _this.logContext);
        } catch (error) {
          if (_this.keysEventUpdateTimeout === undefined) {
            var resendDelay = safeGetRetryAfterMs(error, 5000);
            _this.e2eeLogger.warn("Failed to send m.call.encryption_key, retrying in ".concat(resendDelay), _objectSpread(_objectSpread({}, _this.logContext), {}, {
              error
            }));
            _this.keysEventUpdateTimeout = setTimeout(() => void _this.sendEncryptionKeysEvent(), resendDelay);
          } else {
            _this.e2eeLogger.info("Not scheduling key resend as another re-send is already pending", _objectSpread(_objectSpread({}, _this.logContext), {}, {
              keysEventUpdateTimeout: _this.keysEventUpdateTimeout
            }));
          }
        }
      });
    ...
    _defineProperty(this, "onNewKeyReceived", (userId, deviceId, keyBase64Encoded, index, timestamp) => {
      this.e2eeLogger.info("Received key over key transport ".concat(userId, ":").concat(deviceId, " at index ").concat(index), _objectSpread(_objectSpread({}, this.logContext), {}, {
        timestamp
      }));
    ...
    _defineProperty(this, "onRotateKeyTimeout", () => {
      if (!this.manageMediaKeys) {
        this.e2eeLogger.info("manageMediaKeys is falsy, skipping onRotateKeyTimeout", this.logContext);
        return;
      }
      this.makeNewKeyTimeout = undefined;
      this.e2eeLogger.info("Making new sender key for key rotation", this.logContext);
    ...
    this.e2eeLogger = rootLogger.getChild("[E2EE_FLOW_MX][ENCRYPTION_MANAGER]");
  }
  get logContext() {
    return {
      logSessionId,
      matrixUserId: this.userId,
  ...
  makeNewSenderKey() {
    ...
    this.e2eeLogger.info("Generated new sender key at index " + encryptionKeyIndex, _objectSpread(_objectSpread({}, this.logContext), {}, {
      delay: delayBeforeUse
    }));
  ...
  requestSendCurrentKey() {
    if (!this.manageMediaKeys) {
      this.e2eeLogger.info("manageMediaKeys is falsy, skipping requestSendCurrentKey", this.logContext);
      return;
    }
    this.e2eeLogger.info("Requesting to resend our current keys to the room", this.logContext);
    if (this.lastEncryptionKeyUpdateRequest && this.lastEncryptionKeyUpdateRequest + this.updateEncryptionKeyThrottle > Date.now()) {
      this.e2eeLogger.info("Last encryption key event sent too recently: postponing", _objectSpread(_objectSpread({}, this.logContext), {}, {
  ...
  setEncryptionKey(userId, deviceId, encryptionKeyIndex, encryptionKeyString, timestamp) {
    ...
    this.e2eeLogger.info("Setting encryption key for ".concat(userId, ":").concat(deviceId, " at index ").concat(encryptionKeyIndex), _objectSpread(_objectSpread({}, this.logContext), {}, {
      timestamp
    }));
    ...
    if (existingKeyAtIndex) {
      if (existingKeyAtIndex.timestamp > timestamp) {
        this.e2eeLogger.info("Ignoring new key at index ".concat(encryptionKeyIndex, " for ").concat(participantId, " as it is older than existing known key"), _objectSpread(_objectSpread({}, this.logContext), {}, {
      ...
      if (keysEqual(existingKeyAtIndex.key, keyBin)) {
        this.e2eeLogger.info("Has identical existing key at index ".concat(encryptionKeyIndex, " for ").concat(participantId, ", updating timestamp only"), _objectSpread(_objectSpread({}, this.logContext), {}, {
          timestamp
        }));
    ...
    if (delayBeforeUse) {
      var useKeyTimeout = setTimeout(() => {
        ...
        this.e2eeLogger.info("Delayed-emitting key changed event for ".concat(participantId, " index ").concat(encryptionKeyIndex), _objectSpread(_objectSpread({}, this.logContext), {}, {
          delay: this.useKeyDelay
        }));
